...
The basic workflow will require a secret called `COPILOT_GITHUB_TOKEN` which can be set by navigating to respoitory secrets -> Secrets and variables -> Actions -> Repository secrets. This should be generated by going to user personal access tokens and creating a PAT with Copilot requests: read permissions.

### Permissions

The target Github repo must have the following setting enabled:
